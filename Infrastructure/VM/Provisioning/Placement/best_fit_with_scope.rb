...
begin
  ...
  hosts.each do |h|
    ...
    #############################
    $evm.log("info", "#{@method} - checking host for tags:<#{tags.inspect}>") if @debug
    $evm.log("info", "#{@method} - host tags:<#{h.tags.inspect}>") if @debug
    next unless tags.all? do |key, value|
      $evm.log("info", "#{@method} - key:<#{key}>, value:<#{value}>") if @debug
    ...
    $evm.log("info", "#{@method} - found host:<#{h.name}>") if @debug
