###################################
#
# EVM Automate Method: best_fit_with_platform_and_scope
#
...
def log(level, message)
  @method = 'best_fit_with_platform_and_scope'
...
begin
  log(:info, "EVM Automate Method: <best_fit_with_platform_and_scope> Started")
  #dump_root
  ...
  log(:info, "VM=<#{vm.name}>, Space Required=<#{vm.provisioned_storage}>, group=<#{user.normalized_ldap_group}>") if @debug
  ...
  log(:info, "Sorted host Order:<#{HOST_SORT_ORDER.inspect}> Results:<#{sort_data.inspect}>") if @debug
  ...
  hosts.each do |h|
    ...
    #log(:info, "host:<#{h.inspect}>") if @debug
    log(:info, "host tags:<#{h.tags.inspect}>") if @debug

    #############################
    # Check host for platform tag
    #############################
    log(:info, "checking host <#{h.name}> for assigned VM platform tag <#{vm.platform}>...") if @debug
    next unless h.tagged_with?('platform', vm.platform)

    #############################
    # Only consider hosts that have the required tags
    #############################
    log(:info, "checking host for prov_scope tags:<#{tags.inspect}>...") if @debug
    next unless tags.all? do |key, value|
      log(:info, "key:<#{key}>, value:<#{value}>") if @debug
    ...
    log(:info, "found host:<#{h.name}>") if @debug
    ...
    storages = storages.select do |s|
      ...
      if s.free_space > vm.provisioned_storage + actively_provisioned_space
        ...
        log(:info, "Skipping Datastore:<#{s.name}>, not enough free space for VM:<#{vm.name}>. Available:<#{s.free_space}>, Needs:<#{vm.provisioned_storage}>") if @debug
        false
      end
    end
    ...
    storages = storages.select do |s|
      ...
      if (storage_max_vms == 0) || ((s.vms.size + active_num_vms_for_storage) < storage_max_vms)
        ...
        log(:info, "Skipping Datastore:<#{s.name}>, max number of VMs:<#{s.vms.size + active_num_vms_for_storage}> exceeded") if @debug
        false
      end
    end
    ...
    storages = storages.select do |s|
      ...
      if (storage_max_pct_used == 100) || ((s.v_used_space_percent_of_total + active_pct_of_storage + request_pct_of_storage) < storage_max_pct_used)
        ...
        log(:info, "Skipping Datastore:<#{s.name}> percent of used space #{s.v_used_space_percent_of_total + active_pct_of_storage + request_pct_of_storage} exceeded") if @debug
  ...
  obj = $evm.object
  log(:info, "Selected Host:<#{host.nil? ? "nil" : host.name}>") if @debug
  obj["host"] = host unless host.nil?

  log(:info, "Selected Datastore:<#{storage.nil? ? "nil" : storage.name}>") if @debug
  obj["storage"] = storage unless storage.nil?

  # Set host and storage
  log(:info, "vm=<#{vm.name}> host=<#{host}> storage=<#{storage}>") if @debug
